...
async def get_works(url: str, session: aiohttp.ClientSession):
    ...
    return await asyncio.gather(*(get_work_detail(work_url, session) for work_url in work_urls))
...
async def scrape() -> Tuple[Tuple[str, str, str]]:
    ...
    async with aiohttp.ClientSession() as session:
        jobs: Tuple[Tuple[Tuple[str, str, str]]] = await asyncio.gather(*(get_works(url, session) for url in urls))
...
def create_message(jobs: Tuple[Tuple[str, str, str]], db: _Base):
    ...
    if diff_jobs_urls:
        db.put(data={
            'urls': [*old_jobs_urls, *diff_jobs_urls]
        }, key='jobs_urls')
